...
static void
rfc5335 (FILE * o, const char *value, int quote)
...
static FILE *
pipeemail (pid_t * childp, const char *from)
...
#ifndef	LIGHT
void
email_gpgctx (email_t e, gpgme_ctx_t ctx)
...
void
email_type (email_t e, const char *mimetype)
{
   if (e->mimetype)
   {
      free (e->mimetype);
...
void
email_cid (email_t e, const char *domain)
...
const char *
email_send_save (email_t e, int flags, const char *copy)
{                               // Sending email (copy to a file)
   ...
   void sendmime (email_t e, FILE * o)
   {
      ...
      if (!binary && !longline && e->mimetype
          && ((e->body && e->body->text) || (!strncasecmp (e->mimetype, "text/", 5) && !(flags & EMAIL_PGPMIME))
              || !strncasecmp (e->mimetype, "message/", 8) || !strncasecmp (e->mimetype, "application/pgp", 15)
              || !strncasecmp (e->mimetype, "multipart/encrypted", 19)))
      {                         // Ok to send as is
   ...
   {                            // Sort ctx for GPGME
      ...
      {
         gpgme_error_t cb (void *hook, const char *uid_hint, const char *passphrase_info, int prev_was_bad, int fd)
         {                      // No PIN
            char nl = '\n';
            if (write (fd, &nl, 1) != 1)
               return -1;
   ...
   {                            // Sort list of targets for encryption
      ...
      for (r = e->addresses; r; r = r->next)
      {
         ...
         while (!(gpgerr = gpgme_op_keylist_next (ctx, &key)))
         {
            ...
            if (key->can_encrypt && !key->invalid && !key->expired && !key->revoked && !key->disabled
                && ((key->owner_trust >= GPGME_VALIDITY_MARGINAL && !e->encryptto)
                    || (e->encryptto && (!strcmp (key->subkeys->keyid, r->address) || !strcmp (key->subkeys->fpr, r->address)))))
            {                   // Key looks OK, check UIDs
               gpgme_user_id_t u = NULL;
               if (!e->encryptto)
                  for (u = key->uids; u; u = u->next)
                     if (u->email && !strcmp (u->email, r->address) && !u->revoked && !u->invalid
                         && u->validity >= GPGME_VALIDITY_MARGINAL)
   ...
   if (!err && (flags & (EMAIL_ENCRYPT | EMAIL_SIGN)))
   {
      ...
      {                         // S/MIME
         ...
         {                      // Just sign
            ...
            if (asprintf
                (&n->mimetype, "multipart/signed; protocol=\"application/pgp-signature\"; micalg=\"pgp-%s\"",
                 gpgme_hash_algo_name (signres->signatures->hash_algo)) < 0)
               errx (1, "malloc");
            p2->body = malloc (sizeof (*p2->body));
            memset (p2->body, 0, sizeof (*p2->body));
...
const char *
email_send (email_t e, int flags)
{
...
void
email_address (email_t e, const char *type, const char *email, const char *name)
{
   if (!e || !type || !email)
      return;
   head (e);
...
void
email_subject (email_t e, const char *fmt, ...)
{                               // Set Subject
   char *subject = NULL;
   va_list ap;
   va_start (ap, fmt);
...
email_t
email_add (email_t e, const char *filename, const char *sourcefile, FILE ** op)
{                               // Add new part
   email_t a = NULL;
...
#undef m
   ...
   if (a->body
       && (a == e
           || (e->sublast && e->mimetype && !strcasecmp (e->mimetype, "multipart/alternative") && e->sublast->body
               && e->sublast->body->isinline)))
      a->body->isinline = 1;
   if (e != a)
   {
      // Add this part
...
email_t
email_first (email_t e)
{                               // First email item in email
   return e->sub;
}

email_t
email_alternative (email_t e)
...
void
email_header (email_t e, const char *header, const char *fmt, ...)
{
   char *value = NULL;
   va_list ap;
   va_start (ap, fmt);
